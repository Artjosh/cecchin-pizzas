...
O Mapa Tático lê os pontos de `localizacao_evento` e enquadra os eventos do dia. Para registros antigos com endereço e sem coordenada, busca automaticamente um resultado que corresponda à rua e à cidade e, na gestão, salva o ponto para liberar o planejamento. O operador pode ajustar um endereço impreciso no mapa. As linhas do mapa usam geometria rodoviária de `/api/mapa/rota`, inclusive viagens aprovadas com várias paradas; as rotas do dia são carregadas em lotes. A aba lateral reúne eventos e saídas, com detalhes de cada evento recolhidos para acomodar dias movimentados. Vincular eventos consecutivos exige a mesma equipe e líderes confirmados; o veículo é planejado separadamente.
...
Marketing oferece agenda interna, pedidos, biblioteca e concorrência. A central Instagram tem uma ponte local para a sessão autenticada no navegador, com feed, Stories, perfis, comentários e curtidas; ela usa endpoints web internos não oficiais. Algumas correções recentes de filtragem, avatares e paginação ainda aguardam verificação após recarregar a extensão. Separadamente, OAuth e publicação de conta profissional pela API oficial da Meta estão implementados parcialmente, mas aguardam configuração e validação externa. O upload também guarda mídia na agenda interna e `Registrar manualmente` registra a conclusão humana. Métricas de concorrentes dependem de credenciais e coleta configuradas. Consulte [Instagram na central de marketing](INSTAGRAM_INTEGRACAO.md).
