...
- `MarketingPainel.tsx` agora incorpora `InstagramCentral`: múltiplas contas, OAuth oficial por rotas server-side, leitura de mídia autorizada, composer/agendamento de uma imagem e fila com estados da API. OAuth e postagem não foram executados por falta de credenciais Meta/Storage; a tela local mostra a configuração pendente. O layout foi visto em desktop e celular.
...
- `skills/desenhar-interface/SKILL.md` é o procedimento de UI/UX do projeto. OAuth/feed/publicador têm implementação inicial documentada em [INSTAGRAM_INTEGRACAO.md](INSTAGRAM_INTEGRACAO.md), mas aguardam configuração Meta e deploy dos novos containers; não registrar publicação sem confirmação externa da Meta.
