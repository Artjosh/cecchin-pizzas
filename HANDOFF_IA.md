...
- `skills/desenhar-interface/SKILL.md` é o procedimento de UI/UX do projeto. O plano para contas próprias, feed e publicação reais no Instagram está em [INSTAGRAM_INTEGRACAO.md](INSTAGRAM_INTEGRACAO.md); **nenhum OAuth/publicador foi implementado**. O botão da agenda agora diz “Registrar manualmente”.
...
5. Construir a integração de Instagram na ordem do [plano](INSTAGRAM_INTEGRACAO.md), após configurar aplicativo Meta, callback HTTPS e contas profissionais autorizadas. A prévia atual não é conexão real.
