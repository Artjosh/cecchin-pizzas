...
export default async function Page({ searchParams }: { searchParams: Promise<{ pagina?: string; solicitacao?: string; aba?: string }> }) {
  ...
  const [config, reservas, pedidos] = await Promise.all([
    ...
    consultar<ReservaParaCobrar[]>(`vw_reserva_sem_cobranca?select=id,data_evento,horario,endereco,status,valor_estimado,sinal_estimado,adultos,criancas${filtro}&order=criado_em.desc,id.desc&limit=25&offset=${cobrancas ? 0 : (pagina - 1) * 25}`, sessao.accessToken, { headers: { Prefer: "count=exact" } }),
    consultar<CobrancaInfinitePay[]>(`vw_cobranca_infinitepay_operacao?select=id,solicitacao_id,status,checkout_url,total_aprovado_centavos,valor_centavos,evento_id,erro_codigo,criado_em,avisos_pendentes,ultima_falha&order=criado_em.desc,id.desc&limit=25&offset=${cobrancas ? (pagina - 1) * 25 : 0}`, sessao.accessToken, { headers: { Prefer: "count=exact" } }),
  ]);
  ...
  return <div className="space-y-5"><CabecalhoDoPainel titulo="Pagamentos" descricao="Aprovação de reservas e acompanhamento InfinitePay." />
    {!config.ok || !reservas.ok || !pedidos.ok ? <p role="alert" className="rounded-xl bg-error-container p-4 text-on-error-container">Não foi possível carregar os pagamentos. Confira a disponibilidade do serviço e a aplicação da migration 044.</p> : <>
      ...
      <nav className="flex gap-4"><a href="/admin/pagamentos" className={!cobrancas ? "text-primary font-bold" : ""}>Aprovar reservas ({reservas.total ?? 0})</a><a href="/admin/pagamentos?aba=cobrancas" className={cobrancas ? "text-primary font-bold" : ""}>Cobranças ({pedidos.total ?? 0})</a></nav>
      <div className="grid xl:grid-cols-2 gap-4">{cobrancas ? pedidos.dados?.map(c => <AcompanharCobranca key={c.id + c.status} cobranca={c} />) : reservas.dados?.map(r => <AprovarCobranca key={r.id} reserva={r} habilitado={ambiente && !!config.dados?.[0]?.habilitado} />)}</div>
      {(cobrancas ? pedidos.dados : reservas.dados)?.length === 0 && <p>Nenhum registro nesta página.</p>}
      <Paginacao pagina={pagina} total={(cobrancas ? pedidos.total : reservas.total) ?? 0} porPagina={25} />
    </>}
  </div>;
}
