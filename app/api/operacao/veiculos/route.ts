...
export async function POST(request: NextRequest) {
  ...
  if (!dados || typeof dados.placa !== "string" || !/^[A-Z0-9]{7}$/.test(dados.placa.toUpperCase().replace(/[^A-Z0-9]/g, ""))) return NextResponse.json({ mensagem: "A placa precisa ter exatamente 7 letras ou numeros (ex.: ABC1D23)." }, { status: 400 });
  dados.placa = dados.placa.toUpperCase().replace(/[^A-Z0-9]/g, "");
  if (typeof dados.modelo !== "string" || !dados.modelo.trim() || dados.modelo.length > 100 || !["sedan", "hatch", "utilitario", "van", "outro"].includes(dados.carroceria) || !["nenhum", "mini", "mini_medio", "medio"].includes(dados.forno_maximo) || !["nenhuma", "isopor_pequeno", "isopor_grande"].includes(dados.bebida_maxima) || !Number.isInteger(dados.lugares) || dados.lugares < 1 || dados.lugares > 20 || !Number.isInteger(dados.limite_eventos_levar) || dados.limite_eventos_levar < 1 || dados.limite_eventos_levar > 20) return NextResponse.json({ mensagem: "Confira a ficha do veículo" }, { status: 400 });
  if (sessao.usuario.papel === "staff" && dados.proprietario_id && dados.proprietario_id !== sessao.usuario.id) return NextResponse.json({ mensagem: "Só é possível cadastrar seu veículo" }, { status: 403 });
  const proprietario = sessao.usuario.papel === "staff" ? sessao.usuario.id : (dados.proprietario_id ?? null);
