...
export async function GET(request: NextRequest) {
  ...
  if (!agenda.ok || !solicitacoes.ok || !perfis.ok || !alertas.ok || !concorrentes.ok) return erro("Não foi possível carregar o marketing", 503);
  return NextResponse.json({ agenda: agenda.dados ?? [], solicitacoes: solicitacoes.dados ?? [], perfis: perfis.dados ?? [], alertas: alertas.dados ?? [], concorrentes: concorrentes.dados ?? [] }, { headers: { "Cache-Control": "no-store" } });
}
