...
const camposUso = "id,plano_id,veiculo_id,motorista_id,numero_uso,km_rodados,transportou_material,lavagem_opcao,lavagem_realizada_em,valor_deslocamento_centavos,valor_adicional_centavos,valor_lavagem_centavos,valor_bonus_centavos,estado";
...
export async function GET(request: NextRequest) {
  ...
  if ((tipo !== "viagens" && tipo !== "reembolsos" && tipo !== "lavagens") || !Number.isInteger(pagina) || pagina < 1 || pagina > 10000) return NextResponse.json({ mensagem: "Página inválida" }, { status: 400 });
  ...
  const caminho = tipo === "viagens"
    ...
    : tipo === "lavagens"
      ? `uso_veiculo_particular?select=${camposUso}&lavagem_opcao=eq.lavagem&lavagem_realizada_em=is.null&order=criado_em.asc,id.asc&limit=26&offset=${offset}`
