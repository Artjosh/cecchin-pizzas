...
export function Passo4Resumo() {
  const { depositVal, formatBRL, aceitouTermos, setAceitouTermos } = useReserva();
  return <div className="flex flex-col gap-space-lg">
    ...
    <div className="rounded-xl bg-surface-container-low p-4 space-y-2">
      ...
      <p>Pague o sinal agora. Após a confirmação, sua solicitação será enviada automaticamente para análise de disponibilidade.</p>
      <p className="text-sm text-on-surface-variant">Você pagará no checkout seguro da InfinitePay. As formas disponíveis, parcelas e eventuais encargos serão apresentados lá antes da confirmação. O pagamento não confirma a data por si só.</p>
    </div>
    <label className="flex items-start gap-3">
      ...
      <span className="text-sm text-on-surface-variant">Entendo que esta é uma solicitação, sujeita à análise de disponibilidade e aprovação dos valores pela Central. A reserva será confirmada após pagamento e aprovação da gestão. Em caso de recusa, haverá devolução integral, realizada manualmente.</span>
    </label>
  </div>;
}

export function PreferenciaPagamento() {
  const { paymentMethod, setPaymentMethod } = useReserva();
