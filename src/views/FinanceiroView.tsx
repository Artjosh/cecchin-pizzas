...
export async function FinanceiroView({ paginaEntradas = 1, paginaDespesas = 1, paginaAcertos = 1, paginaEscalas = 1 }: { paginaEntradas?: number; paginaDespesas?: number; paginaAcertos?: number; paginaEscalas?: number } = {}) {
  ...
  const [caixaR, resumoR, acertosR, escalasR, cartoesR, faturasR, pessoasR, planosR, veiculosR, usosR, lavagensR, usosResumoR, regraVeiculoR] = await Promise.all([
    consultar<MovimentoHoje[]>(`vw_caixa_hoje?select=origem_id,dia,natureza,descricao,valor,taxa_pendente&dia=eq.${hoje}&order=natureza.asc,descricao.asc,valor.asc,origem_id.asc&limit=50`, sessao.accessToken),
    consultar<ResumoCaixa[]>(`vw_caixa_resumo_dia?select=movimentos,entradas,saidas,saldo,taxas_pendentes&dia=eq.${hoje}&limit=1`, sessao.accessToken),
