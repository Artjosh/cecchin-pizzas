...
interface SolicitacaoReserva { canal: "site" | "whatsapp"; nome_contato: string | null; telefone_contato: string | null; id: string; status: string; data_evento: string; horario: string; endereco: string; adultos: number; valor_estimado: string | number; sinal_estimado: string | number; criado_em: string; usuario: { nome: string; email: string | null } | null; }
...
export async function NotificacoesView() {
  ...
  const [configR, regrasR, filaR, reservasR] = await Promise.all([
    ...
    consultar<SolicitacaoReserva[]>("solicitacao_reserva?select=id,canal,nome_contato,telefone_contato,status,data_evento,horario,endereco,adultos,valor_estimado,sinal_estimado,criado_em,usuario(nome,email)&status=in.(enviada,em_analise,aguardando_pagamento)&order=criado_em.asc&limit=50", sessao.accessToken),
  ]);
