  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
        <h1 className="text-2xl font-bold text-stone-900 mb-2 flex items-center gap-2">
          <ChefHat className="text-amber-600" />
          Checklist Pré-Evento
        </h1>
        <p className="text-stone-500 mb-6">Evento: Aniversário 15 Anos - Marina</p>
        
        <div className="mb-6">
          <div className="flex justify-between text-sm mb-2 font-medium">
            <span className="text-stone-700">Progresso</span>
          ...
          {items.map(item => (
            <motion.div 
              ...
              className={`flex items-center gap-4 p-4 rounded-lg border cursor-pointer transition-colors ${
                item.done ? 'bg-stone-50 border-stone-200' : 'bg-white border-amber-200 hover:border-amber-300 shadow-sm'
              }`}
            >
              ...
              <span className={`font-medium ${item.done ? 'text-stone-400 line-through' : 'text-stone-700'}`}>
                {item.text}
              </span>
            </motion.div>
          ))}
