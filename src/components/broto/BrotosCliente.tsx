...
type Dados = { cliente: Cliente | null; produtos: Produto[]; precos: Preco[]; pedidos: Pedido[] };
...
export function BrotosCliente() {
  ...
  const carregar = useCallback(async () => {
    setCarregando(true);
    try {
      const resposta = await fetch("/api/broto/cliente", { cache: "no-store" });
