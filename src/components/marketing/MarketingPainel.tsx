...
import { CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, Clock3, Plus, RefreshCw, Send, Trash2 } from "lucide-react";
...
type Alerta = { agenda_id: string; criado_em: string; lido_em: string | null; agenda_marketing: { agendado_para: string; titulo: string; situacao: string } | null };
...
const campo = "w-full rounded-xl bg-surface-container-high px-3 py-2 text-on-surface outline-none focus:ring-2 focus:ring-primary/50";
const botao = "inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 font-label-md transition-colors disabled:cursor-not-allowed disabled:opacity-50";
...
export function MarketingPainel({ usuarioId, gestor, pessoas }: { usuarioId: string; gestor: boolean; pessoas: Pessoa[] }) {
  ...
  const [indiceItemDia, setIndiceItemDia] = useState(0);
  ...
  const alertasPendentes = new Set((dados?.alertas ?? []).filter((a) => a.agenda_marketing?.situacao === "planejado").map((a) => a.agenda_id));
  useEffect(() => {
  ...
  const agendaPorDia = useMemo(() => {
    ...
    for (const item of dados?.agenda ?? []) {
      ...
      if (item.situacao !== "cancelado") grupos.set(chave, [...(grupos.get(chave) ?? []), item]);
    }
    return grupos;
  }, [dados]);
  const dias = Array.from({ length: 7 }, (_, indice) => somarDias(semana, indice));
  const itensDiaAberto = diaAberto ? agendaPorDia.get(diaAberto) ?? [] : [];
  const indiceAtualDia = Math.min(indiceItemDia, Math.max(0, itensDiaAberto.length - 1));
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    {removerId && <div role="dialog" aria-modal="true" aria-label="Remover compromisso" className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-sm rounded-2xl bg-surface-container p-5 shadow-xl"><h2 className="text-lg font-semibold">Remover da agenda?</h2><p className="mt-2 text-sm text-on-surface-variant">O item sai da agenda. O arquivo permanece na biblioteca.</p><div className="mt-5 flex justify-end gap-2"><button type="button" className={`${botao} bg-surface-container-high`} onClick={() => setRemoverId(null)}>Voltar</button><button type="button" disabled={salvando} className={`${botao} bg-error text-on-error`} onClick={async () => { if (await enviar({ acao: "remover_agenda", id: removerId }, "Item removido da agenda")) setRemoverId(null); }}>Remover</button></div></div></div>}
    ...
    {aba !== "biblioteca" && <div className={`grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-2 gap-y-1 md:flex md:flex-wrap md:items-end md:justify-between md:gap-3 ${aba === "agenda" ? "" : "grid-cols-1"}`}><div className="min-w-0"><h2 className="text-lg font-semibold tracking-tight text-on-surface md:text-2xl">{{ agenda: "Agenda semanal", pedidos: "Pedidos de conte\u00fado", concorrencia: "Concorr\u00eancia", equipe: "Equipe de marketing" }[aba]}</h2><p className="mt-0.5 text-xs leading-tight text-on-surface-variant md:mt-1 md:text-sm">{{ agenda: "Planeje publica\u00e7\u00f5es e acompanhe os sete dias da semana.", pedidos: "Organize as solicita\u00e7\u00f5es e acompanhe cada entrega.", concorrencia: "Acompanhe os perfis e as m\u00e9tricas dispon\u00edveis.", equipe: "Defina quem pode trabalhar no conte\u00fado da equipe." }[aba]}</p></div>{aba === "agenda" && <div className="row-span-2 flex w-[min(38vw,10rem)] shrink-0 flex-col items-end gap-1 md:row-span-1 md:w-auto md:flex-row md:items-center md:gap-3"><span className="shrink-0 text-xs font-semibold text-primary md:text-sm">{diasProgramados} / 7 dias</span>{!!alertasPendentes.size && <div className="w-full overflow-hidden rounded-lg bg-primary-container text-on-primary-container md:w-auto md:rounded-xl">
      <button type="button" aria-expanded={alertasAbertos} aria-controls="lista-alertas-agenda" onClick={() => setAlertasAbertos((aberto) => !aberto)} className="flex h-7 w-full items-center justify-between gap-1.5 px-2 text-left md:h-8 md:gap-2 md:px-3">
        <span aria-live="polite" className="min-w-0 truncate text-[11px] font-semibold md:text-xs">{alertasPendentes.size} {alertasPendentes.size === 1 ? "pend\u00eancia de confirma\u00e7\u00e3o" : "pend\u00eancias de confirma\u00e7\u00e3o"}</span>
        <span className="flex shrink-0 items-center gap-1 text-[10px] md:text-[11px]">{alertasAbertos ? "Ocultar" : "Ver"}<ChevronDown className={`h-3.5 w-3.5 transition-transform ${alertasAbertos ? "rotate-180" : ""}`} /></span>
      </button>
      <div id="lista-alertas-agenda" hidden={!alertasAbertos} className="flex flex-wrap gap-2 px-2 pb-2 md:px-3">
        {(dados?.alertas ?? []).filter((alerta) => alerta.agenda_marketing && alertasPendentes.has(alerta.agenda_id)).slice(0, 5).map((alerta) => <button key={alerta.agenda_id} type="button" className="rounded-lg bg-surface-container-lowest px-3 py-1.5 text-left text-sm text-on-surface hover:bg-surface-container-high" onClick={() => { const dataAlerta = dataSaoPaulo(new Date(alerta.agenda_marketing!.agendado_para)); const agendaDoDia = agendaPorDia.get(dataAlerta) ?? []; setIndiceItemDia(Math.max(0, agendaDoDia.findIndex((item) => item.id === alerta.agenda_id))); setAlertasAbertos(false); setAlvoAlerta(alerta.agenda_id); setDiaAberto(dataAlerta); setSemana(inicioSemana(new Date(alerta.agenda_marketing!.agendado_para))); setAba("agenda"); setMobilePreview(false); }}>{alerta.agenda_marketing!.titulo} {"\u00b7"} {rotuloData(alerta.agenda_marketing!.agendado_para)}</button>)}
        {alertasPendentes.size > 5 && <p className="mt-2 w-full text-xs">Mostrando os cinco lembretes mais recentes.</p>}
      </div>
    </div>}</div>}</div>}
    ...
    {aba === "agenda" && <section aria-label="Agenda semanal" className="space-y-3">
      <div className="p-0.5"><div className="grid min-w-0 grid-cols-7 gap-1">{dias.map((dia) => { const itens = agendaPorDia.get(dia) ?? []; const ativo = diaAberto === dia; const data = new Date(`${dia}T12:00:00Z`); return <button key={dia} type="button" aria-expanded={ativo} aria-label={`${new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "numeric", month: "long", timeZone: "UTC" }).format(data)}: ${itens.length} ${itens.length === 1 ? "compromisso" : "compromissos"}`} onClick={() => { setIndiceItemDia(0); setDiaAberto(ativo ? null : dia); }} className={`flex h-14 min-w-0 flex-col items-center justify-center rounded-lg px-0 text-center transition-colors sm:h-16 sm:rounded-xl sm:px-1 ${ativo ? "bg-primary-container text-on-primary-container ring-2 ring-inset ring-primary" : "bg-surface-container-lowest text-on-surface hover:bg-surface-container-high"}`}><span className="text-[9px] capitalize leading-tight text-on-surface-variant sm:text-xs">{new Intl.DateTimeFormat("pt-BR", { weekday: "short", timeZone: "UTC" }).format(data)}</span><strong className="text-lg leading-tight sm:text-xl">{new Intl.DateTimeFormat("pt-BR", { day: "2-digit", timeZone: "UTC" }).format(data)}</strong><span className="whitespace-nowrap text-[8px] leading-tight sm:text-xs">{itens.length} {itens.length === 1 ? "item" : "itens"}</span></button>; })}</div></div>
      {diaAberto && dias.includes(diaAberto) && <div className="rounded-xl bg-surface-container-lowest p-3">
        <div className="mb-2 flex min-w-0 items-center gap-1">
          <h3 className="min-w-0 flex-1 truncate text-sm font-semibold">{new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "2-digit", month: "long", timeZone: "UTC" }).format(new Date(`${diaAberto}T12:00:00Z`))}</h3>
          {itensDiaAberto.length > 1 && <div className="flex shrink-0 items-center gap-1">
            <button type="button" aria-label="Compromisso anterior" onClick={() => setIndiceItemDia((indiceAtualDia - 1 + itensDiaAberto.length) % itensDiaAberto.length)} className="flex h-7 w-7 items-center justify-center rounded-lg text-on-surface-variant hover:bg-surface-container"><ChevronLeft className="h-4 w-4" /></button>
            <span aria-live="polite" className="min-w-8 text-center text-xs tabular-nums text-on-surface-variant">{indiceAtualDia + 1}/{itensDiaAberto.length}</span>
            <button type="button" aria-label={"Pr\u00f3ximo compromisso"} onClick={() => setIndiceItemDia((indiceAtualDia + 1) % itensDiaAberto.length)} className="flex h-7 w-7 items-center justify-center rounded-lg text-on-surface-variant hover:bg-surface-container"><ChevronRight className="h-4 w-4" /></button>
          </div>}
          {(souMarketing || gestor) && <button type="button" className="shrink-0 whitespace-nowrap px-1 text-[11px] font-semibold text-primary hover:underline" onClick={() => { setFormAgenda((atual) => ({ ...atual, agendado_para: `${diaAberto}T12:00` })); document.getElementById("formulario-agenda-marketing")?.scrollIntoView({ behavior: "smooth", block: "center" }); }}>Programar este dia</button>}
          <button type="button" onClick={() => { setDiaAberto(null); setIndiceItemDia(0); }} className="shrink-0 px-1 text-xs text-on-surface-variant hover:text-on-surface">Fechar</button>
        </div>
        {itensDiaAberto.length ? <div className="space-y-2">{itensDiaAberto.slice(indiceAtualDia, indiceAtualDia + 1).map((item) => <article key={item.id} id={`marketing-agenda-${item.id}`} className={`rounded-xl bg-surface-container p-2.5 ${alertasPendentes.has(item.id) ? "ring-2 ring-inset ring-primary" : ""}`}><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-primary-container px-2 py-0.5 text-xs text-on-primary-container">{item.categoria === "tarefa" ? "Tarefa" : item.categoria}</span><strong className="text-sm">{item.titulo}</strong><span className="ml-auto text-xs text-on-surface-variant">{rotuloData(item.agendado_para).split(" ").slice(-1)}</span></div><p className="mt-1 text-xs text-on-surface-variant">{nome(item.responsavel_id)} · {item.categoria === "tarefa" && item.situacao === "publicado" ? "concluída" : item.situacao}</p>{item.descricao_conteudo && <p className="mt-2 text-sm text-on-surface-variant">{item.descricao_conteudo}</p>}{item.midia_caminho && <a className="mt-2 inline-block text-xs font-semibold text-primary underline" href={`/api/operacao/marketing/midia?caminho=${encodeURIComponent(item.midia_caminho)}`} target="_blank" rel="noreferrer">Abrir foto ou vídeo</a>}{item.responsavel_id === usuarioId && item.situacao !== "publicado" && item.situacao !== "cancelado" && <div className="mt-2 flex flex-wrap items-center gap-2">{item.situacao === "planejado" && <button disabled={salvando} className={`${botao} bg-surface-container-high text-xs`} onClick={() => void enviar({ acao: "confirmar", id: item.id }, "Agendamento confirmado")}>Postagem automatizada</button>}<span className="group relative inline-flex">{item.categoria !== "tarefa" && <span role="tooltip" className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 hidden w-56 -translate-x-1/2 rounded-lg bg-surface-container-high px-3 py-2 text-xs font-normal text-on-surface shadow-lg group-hover:block group-focus-within:block">{"Registra na agenda; n\u00e3o publica no Instagram."}</span>}<button disabled={salvando} aria-label={item.categoria === "tarefa" ? "Concluir tarefa" : "Registar como manual. Registra na agenda; n\u00e3o publica no Instagram."} className={`${botao} bg-primary text-on-primary text-xs`} onClick={() => void enviar({ acao: "publicar", id: item.id }, item.categoria === "tarefa" ? "Tarefa conclu\u00edda" : "Publica\u00e7\u00e3o registrada")}>{item.categoria === "tarefa" ? "Concluir tarefa" : <>Registar como manual<span aria-hidden="true" className="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full border border-on-primary/70 text-[10px] font-bold leading-none">?</span></>}</button></span></div>}{(gestor || item.responsavel_id === usuarioId) && <button type="button" disabled={salvando} onClick={() => setRemoverId(item.id)} className="mt-2 inline-flex items-center gap-1 text-xs text-on-surface-variant hover:text-error"><Trash2 className="h-3.5 w-3.5" />Remover da agenda</button>}</article>)}</div> : <p className="text-sm text-on-surface-variant">{"Sem conte\u00fado programado."}</p>}
      </div>}
    </section>}
