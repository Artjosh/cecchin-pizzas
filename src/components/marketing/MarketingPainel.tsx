...
export function MarketingPainel({ usuarioId, gestor, pessoas }: { usuarioId: string; gestor: boolean; pessoas: Pessoa[] }) {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    ...
    <nav aria-label="Áreas do marketing" className="flex gap-1 overflow-x-auto rounded-2xl border border-outline-variant/30 bg-surface-container-low p-1.5 text-sm">
      {([ ["agenda","Agenda"],["pedidos","Pedidos"],["biblioteca","Biblioteca"],["concorrencia","Concorrência"],["equipe","Equipe"] ] as const).map(([id,nome])=><button key={id} type="button" onClick={()=>{setAba(id);setMobilePreview(false);}} aria-current={aba===id?"page":undefined} className={`shrink-0 rounded-xl px-4 py-2.5 transition-colors ${aba===id?"bg-primary-container font-semibold text-on-primary-container shadow-sm":"text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface"}`}>{nome}</button>)}
    ...
    <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-2">
      ...
      <div className={`${mobilePreview?"hidden lg:block":""} h-full min-h-0 space-y-5 overflow-y-auto rounded-2xl border border-outline-variant/30 bg-surface-container-low p-4 lg:p-5`}>
    {aba !== "biblioteca" && <div className="border-b border-outline-variant/30 pb-4"><p className="text-xs font-semibold uppercase tracking-widest text-primary">Organização interna</p><div className="mt-1 flex items-end justify-between gap-3"><div><h2 className="font-title-lg text-on-surface">{{ agenda: "Agenda semanal", pedidos: "Pedidos de conteúdo", concorrencia: "Concorrência", equipe: "Equipe de marketing" }[aba]}</h2><p className="mt-1 text-sm text-on-surface-variant">{{ agenda: "Planeje publicações e acompanhe os sete dias da semana.", pedidos: "Organize as solicitações e acompanhe cada entrega.", concorrencia: "Acompanhe os perfis e as métricas disponíveis.", equipe: "Defina quem pode trabalhar no conteúdo da equipe." }[aba]}</p></div>{aba === "agenda" && <span className="shrink-0 rounded-full bg-primary-container px-3 py-1 text-xs font-semibold text-on-primary-container">{diasProgramados} / 7 dias</span>}</div></div>}
    ...
    {gestor && aba === "equipe" && <section className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-4"><div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="font-title-md">Pessoas com acesso</h3><p className="text-sm text-on-surface-variant">Selecione quem pode receber pedidos e assumir a agenda.</p></div><span className="rounded-full bg-primary-container px-3 py-1 text-xs font-semibold text-on-primary-container">{ids.size} {ids.size === 1 ? "integrante" : "integrantes"}</span></div><div className="grid gap-2 sm:grid-cols-2">{pessoas.map((pessoa) => <button key={pessoa.id} type="button" disabled={salvando} onClick={() => void enviar({ acao: "atribuir", usuario_id: pessoa.id, ativo: !ids.has(pessoa.id) }, ids.has(pessoa.id) ? `${pessoa.nome} removido do marketing` : `${pessoa.nome} incluído no marketing`)} className={`flex items-center justify-between rounded-xl border p-3 text-left text-sm transition-colors disabled:opacity-50 ${ids.has(pessoa.id) ? "border-primary/50 bg-primary-container text-on-primary-container" : "border-outline-variant/40 bg-surface-container-low text-on-surface hover:bg-surface-container-high"}`}><span className="truncate">{pessoa.nome}</span><span className="ml-2 text-xs">{ids.has(pessoa.id) ? <Check className="h-4 w-4" /> : "Adicionar"}</span></button>)}</div></section>}
