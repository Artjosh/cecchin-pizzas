...
import { CalendarDays, Check, Clock3, Plus, RefreshCw, Send, Trash2 } from "lucide-react";
...
export function MarketingPainel({ usuarioId, gestor, pessoas }: { usuarioId: string; gestor: boolean; pessoas: Pessoa[] }) {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3">
    ...
    {aba === "agenda" && !!alertasPendentes.size && <div role="status" className="flex flex-wrap items-center gap-2 rounded-xl bg-primary-container px-3 py-2 text-on-primary-container"><p className="text-xs font-semibold">{alertasPendentes.size} {alertasPendentes.size === 1 ? "compromisso precisa" : "compromissos precisam"} da sua confirmação.</p><div className="flex flex-wrap gap-2">{(dados?.alertas ?? []).filter((alerta) => alerta.agenda_marketing && alertasPendentes.has(alerta.agenda_id)).slice(0, 5).map((alerta) => <button key={alerta.agenda_id} type="button" className="rounded-lg bg-surface-container-lowest px-3 py-1.5 text-left text-sm text-on-surface hover:bg-surface-container-high" onClick={() => { setAlvoAlerta(alerta.agenda_id); setDiaAberto(dataSaoPaulo(new Date(alerta.agenda_marketing!.agendado_para))); setSemana(inicioSemana(new Date(alerta.agenda_marketing!.agendado_para))); setAba("agenda"); setMobilePreview(false); }}>{alerta.agenda_marketing!.titulo} · {rotuloData(alerta.agenda_marketing!.agendado_para)}</button>)}</div>{alertasPendentes.size > 5 && <p className="mt-2 text-xs">Mostrando os cinco lembretes mais recentes.</p>}</div>}
    ...
    {aba === "agenda" && <section aria-label="Agenda semanal" className="space-y-3">
      <div className="overflow-x-auto p-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><div className="grid min-w-[40rem] grid-cols-7 gap-1.5">{dias.map((dia) => { const itens = agendaPorDia.get(dia) ?? []; const ativo = diaAberto === dia; const data = new Date(`${dia}T12:00:00Z`); return <button key={dia} type="button" aria-expanded={ativo} aria-label={`${new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "numeric", month: "long", timeZone: "UTC" }).format(data)}: ${itens.length} ${itens.length === 1 ? "compromisso" : "compromissos"}`} onClick={() => setDiaAberto(ativo ? null : dia)} className={`flex h-16 min-w-0 flex-col items-center justify-center rounded-xl px-1 text-center transition-colors ${ativo ? "bg-primary-container text-on-primary-container ring-2 ring-inset ring-primary" : "bg-surface-container-lowest text-on-surface hover:bg-surface-container-high"}`}><span className="text-xs capitalize text-on-surface-variant">{new Intl.DateTimeFormat("pt-BR", { weekday: "short", timeZone: "UTC" }).format(data)}</span><strong className="text-xl leading-tight">{new Intl.DateTimeFormat("pt-BR", { day: "2-digit", timeZone: "UTC" }).format(data)}</strong><span className="text-xs">{itens.length} {itens.length === 1 ? "item" : "itens"}</span></button>; })}</div></div>
      {diaAberto && dias.includes(diaAberto) && <div className="rounded-xl bg-surface-container-lowest p-3"><div className="mb-2 flex items-center justify-between gap-2"><h3 className="font-semibold">{new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "2-digit", month: "long", timeZone: "UTC" }).format(new Date(`${diaAberto}T12:00:00Z`))}</h3><button type="button" onClick={() => setDiaAberto(null)} className="text-xs text-on-surface-variant hover:text-on-surface">Fechar</button></div>
        {itensDiaAberto.length ? <div className="space-y-2">{itensDiaAberto.map((item) => <article key={item.id} id={`marketing-agenda-${item.id}`} className={`rounded-xl bg-surface-container p-2.5 ${alertasPendentes.has(item.id) ? "ring-2 ring-inset ring-primary" : ""}`}><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-primary-container px-2 py-0.5 text-xs text-on-primary-container">{item.categoria === "tarefa" ? "Tarefa" : item.categoria}</span><strong className="text-sm">{item.titulo}</strong><span className="ml-auto text-xs text-on-surface-variant">{rotuloData(item.agendado_para).split(" ").slice(-1)}</span></div><p className="mt-1 text-xs text-on-surface-variant">{nome(item.responsavel_id)} · {item.categoria === "tarefa" && item.situacao === "publicado" ? "concluída" : item.situacao}</p>{item.descricao_conteudo && <p className="mt-2 text-sm text-on-surface-variant">{item.descricao_conteudo}</p>}{item.midia_caminho && <a className="mt-2 inline-block text-xs font-semibold text-primary underline" href={`/api/operacao/marketing/midia?caminho=${encodeURIComponent(item.midia_caminho)}`} target="_blank" rel="noreferrer">Abrir foto ou vídeo</a>}{item.responsavel_id === usuarioId && item.situacao !== "publicado" && item.situacao !== "cancelado" && <div className="mt-2 flex flex-wrap items-center gap-2">{item.situacao === "planejado" && <button disabled={salvando} className={`${botao} bg-surface-container-high text-xs`} onClick={() => void enviar({ acao: "confirmar", id: item.id }, "Compromisso confirmado")}>Confirmar compromisso</button>}<button disabled={salvando} className={`${botao} bg-primary text-on-primary text-xs`} onClick={() => void enviar({ acao: "publicar", id: item.id }, item.categoria === "tarefa" ? "Tarefa concluída" : "Publicação registrada")}>{item.categoria === "tarefa" ? "Concluir tarefa" : "Marcar como publicado"}</button>{item.categoria !== "tarefa" && <span className="text-xs text-on-surface-variant">Registra na agenda; não publica no Instagram.</span>}</div>}{(gestor || item.responsavel_id === usuarioId) && <button type="button" disabled={salvando} onClick={() => setRemoverId(item.id)} className="mt-2 inline-flex items-center gap-1 text-xs text-on-surface-variant hover:text-error"><Trash2 className="h-3.5 w-3.5" />Remover da agenda</button>}</article>)}</div> : <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-on-surface-variant">Sem conteúdo programado.</p>{(souMarketing || gestor) && <button type="button" className="text-xs font-semibold text-primary" onClick={() => { setFormAgenda((atual) => ({ ...atual, agendado_para: `${diaAberto}T12:00` })); document.getElementById("formulario-agenda-marketing")?.scrollIntoView({ behavior: "smooth", block: "center" }); }}>Programar este dia</button>}</div>}
      </div>}
    </section>}
