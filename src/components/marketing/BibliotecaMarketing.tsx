...
export function BibliotecaMarketing() {
  ...
  return <section aria-labelledby="biblioteca-titulo" className="space-y-8">
    ...
    <div className="space-y-4 rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-5"><div className="flex items-center justify-between"><h3 className="text-lg font-semibold">Arquivos</h3><span className="text-xs text-on-surface-variant">Página {pagina + 1}</span></div>
      ...
      {!!arquivos.length && <div className="divide-y divide-outline-variant/20">{arquivos.map((item) => { const url = `/api/operacao/marketing/midia?caminho=${encodeURIComponent(item.caminho)}`; return <a key={item.caminho} href={url} target="_blank" rel="noreferrer" className="group flex min-w-0 items-center gap-3 py-3 hover:text-primary"><span className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-surface-container-high text-primary">{item.tipo?.startsWith("image/") ? <img src={url} alt="" loading="lazy" className="h-full w-full object-cover" /> : item.tipo?.startsWith("video/") ? <video src={url} muted preload="metadata" className="h-full w-full object-cover" /> : <Film className="h-5 w-5" />}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{item.caminho.split("/").at(-1)}</strong><span className="text-xs text-on-surface-variant">{item.criado_em ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeZone: "America/Sao_Paulo" }).format(new Date(item.criado_em)) : "Data indisponível"}{item.tamanho != null ? ` · ${(item.tamanho / 1048576).toFixed(1)} MB` : ""}</span></span><ArrowUpRight className="h-4 w-4 shrink-0 text-on-surface-variant" /></a>; })}</div>}
