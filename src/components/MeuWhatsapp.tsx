...
export function MeuWhatsapp({ telefoneInicial, habilitadoInicial }: { telefoneInicial: string; habilitadoInicial: boolean }) {
  ...
  async function salvar() {
    ...
    try {
      const r = await fetch("/api/operacao/notificacoes", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ telefone, receber_email: true, receber_whatsapp: habilitado, avisar_evento_novo: true, avisar_escala: true, avisar_disciplina: true }) });
      const dados = (await r.json().catch(() => ({}))) as { mensagem?: string };
      if (!r.ok) throw new Error(dados.mensagem ?? "Não foi possível salvar.");
      setEstado("WhatsApp vinculado. Você pode escrever MINHA RESERVA no bot.");
