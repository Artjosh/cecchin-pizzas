...
export default function RoleSwitcher() {
  ...
  return (
    /*
     * z-20 de proposito, abaixo do painel de reserva (z-30). Na tela de
     * contratacao o painel cobre a viewport inteira no celular; com z-50
     * este seletor ficava por cima do total e do botao de pagamento. Em
     * z-20 ele aparece quando ha mapa para ver e some quando nao ha.
     */
    <div className="fixed bottom-4 left-4 z-20 flex flex-col items-start">
            ...
            <div className="flex flex-col py-1">
              {roles.map((role) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 ${isActive ? role.color : "text-on-surface-variant/70"}`}
                    />
                    {role.label}
                  </button>
                );
      ...
      <button
        ...
        className="flex items-center gap-2 bg-inverse-surface text-inverse-on-surface px-4 py-3 rounded-full shadow-lg hover:bg-inverse-surface transition-colors"
      >
        <CurrentIcon
          className={`w-5 h-5 ${currentRoleConfig?.color.replace("text-", "text-").replace("-500", "-400")}`}
        />
        <span className="text-sm font-medium hidden sm:inline-block">
          Modo: {currentRoleConfig?.label}
        </span>
      </button>
    </div>
  );
