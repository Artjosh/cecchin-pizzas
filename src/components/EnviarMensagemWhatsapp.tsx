...
export function EnviarMensagemWhatsapp({ telefones, fixarTelefone = false, aoEnviar }: { telefones: string[]; fixarTelefone?: boolean; aoEnviar?: () => void }) {
  ...
  async function enviar(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (aoEnviar) aoEnviar(); else router.refresh();
    } catch (erro) {
  ...
  if (fixarTelefone) return (
    <form onSubmit={enviar} className="flex flex-col gap-2 rounded-xl bg-surface-container-low p-3">
      <label className="text-sm">Mensagem<textarea value={texto} onChange={(event) => setTexto(event.target.value)} required maxLength={4096} rows={2} className="mt-1 block w-full resize-none rounded-lg bg-surface p-2 text-on-surface" /></label>
      <div className="flex h-10 min-w-0 items-center gap-2"><button type="submit" disabled={estado === "enviando"} className="flex h-10 min-w-32 shrink-0 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-on-primary disabled:opacity-60"><Send className="h-4 w-4" />{estado === "enviando" ? "Enfileirando…" : "Enviar"}</button>{mensagem && <span role="status" title={mensagem} className={`min-w-0 truncate text-sm ${estado === "erro" ? "text-error" : "text-primary"}`}>{mensagem}</span>}</div>
    </form>
  );
