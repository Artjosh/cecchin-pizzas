...
export function LogisticaPainel({ diaInicial, compacto = false, versaoLocais = 0, aoAtualizarMapa }: { diaInicial?: string; compacto?: boolean; versaoLocais?: number; aoAtualizarMapa?: (resumo: ResumoMapaLogistico | null) => void } = {}) {
  ...
  return <section className={compacto ? "space-y-3 p-3" : "space-y-4 rounded-2xl bg-surface-container-low p-4 sm:p-5"}>
    ...
    {dados && <>
      ...
      {compacto && <div className="flex flex-wrap gap-1.5 text-xs"><button type="button" aria-expanded={mostrarViagem} onClick={() => setMostrarViagem((atual) => !atual)} className="rounded-lg bg-surface-container px-2.5 py-1.5">Rota com paradas</button><button type="button" aria-expanded={mostrarCarros} onClick={() => setMostrarCarros((atual) => !atual)} className="rounded-lg bg-surface-container px-2.5 py-1.5">Filtrar carros</button><button type="button" aria-expanded={mostrarDupla} onClick={() => setMostrarDupla((atual) => !atual)} className="rounded-lg bg-surface-container px-2.5 py-1.5">Mesma equipe em 2 eventos</button></div>}
      {planejamento.pendencias.length > 0 && <div role="status" className="space-y-2 rounded-xl bg-primary/10 p-3 text-sm"><button type="button" aria-expanded={!compacto || mostrarPendencias} onClick={() => setMostrarPendencias((atual) => !atual)} className="text-left font-semibold">Ajuda necessária em {planejamento.pendencias.length} {planejamento.pendencias.length === 1 ? "evento" : "eventos"}</button>{(!compacto || mostrarPendencias) && <ul className="space-y-1">{planejamento.pendencias.map((pendencia) => <li key={pendencia.eventoId}><span className="font-semibold">{dados.eventos.find((evento) => evento.id === pendencia.eventoId)?.cliente_nome ?? "Evento"}:</span> {pendencia.motivo}</li>)}</ul>}</div>}
      ...
      {eventosParaViagem.length >= 2 && (!compacto || mostrarViagem) && <section className="space-y-3 rounded-2xl bg-surface-container p-4" aria-label="Viagem com várias paradas">
      ...
      {dados.eventos.length === 0 ? <p className="rounded-xl bg-surface-container p-5 text-sm text-on-surface-variant">Nenhum evento confirmado para este dia.</p> : <div className={compacto ? "grid gap-3" : "grid gap-3 md:grid-cols-2 xl:grid-cols-3"}>{dados.eventos.map((evento) => {
        const requisito = dados.requisitos.find((item) => item.evento_id === evento.id);
        const rota = rotaAtual(dados, evento.id);
        const dupla = dados.duplos.find((item) => item.primeiro_evento_id === evento.id || item.segundo_evento_id === evento.id);
