...
export type ResumoMapaLogistico = { planos: Array<Pick<Plano, "id" | "evento_id" | "evento_duplo_id" | "situacao" | "modo" | "veiculo_id">>; paradas: Array<Pick<Parada, "plano_id" | "evento_id" | "ordem">>; duplos: Array<Pick<Duplo, "id" | "primeiro_evento_id" | "segundo_evento_id">> };
...
export function LogisticaPainel({ diaInicial, compacto = false, eventoFoco, versaoLocais = 0, aoAtualizarMapa }: { diaInicial?: string; compacto?: boolean; eventoFoco?: string; versaoLocais?: number; aoAtualizarMapa?: (resumo: ResumoMapaLogistico | null) => void } = {}) {
  ...
  const [eventoExpandido, setEventoExpandido] = useState<string | null>(eventoFoco ?? null);
  const [mostrarCarros, setMostrarCarros] = useState(false);
  const [mostrarViagem, setMostrarViagem] = useState(false);
  const [mostrarDupla, setMostrarDupla] = useState(false);
  ...
  }, [dia, revisao, versaoLocais]);
  useEffect(() => { aoAtualizarMapa?.(dados ? { planos: dados.planos, paradas: dados.paradas, duplos: dados.duplos } : null); }, [dados, aoAtualizarMapa]);
  ...
  const sugestoesViagem = dados ? veiculosSelecionados.filter((carro) => carro.disponivel && carro.limiteLevar >= 2).flatMap((carro) => {
    const original = dados.veiculos.find((item) => item.id === carro.id);
    const grupo = original ? sugerirGrupoViagem(dados, original, eventosParaViagem) : [];
    return grupo.length >= 2 ? [{ carro, grupo }] : [];
  }).sort((a, b) => b.grupo.length - a.grupo.length).slice(0, 4) : [];
  ...
  const eventosExibidos = dados?.eventos.filter((item) => !eventoFoco || item.id === eventoFoco) ?? [];
  const pendenciasExibidas = planejamento.pendencias.filter((item) => !eventoFoco || item.eventoId === eventoFoco);
  return <section className={compacto ? "space-y-3 p-3" : "space-y-4 rounded-2xl bg-surface-container-low p-4 sm:p-5"}>
    {!compacto && <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="font-headline-sm text-headline-sm">Planejar saídas</h2><p className="text-sm text-on-surface-variant">Confira carga, trajeto e carro antes de aprovar a logística.</p></div><label className="grid gap-1 text-xs">Dia dos eventos<input className={campo} type="date" value={dia} onChange={(evento) => { setDia(evento.target.value); setSelecionados(null); setOpcaoPorEvento({}); }} /></label></div>}
    ...
    {dados && <>
      <div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-surface-container px-3 py-1">{eventoFoco ? "Carro deste evento" : `${dados.eventos.length} eventos`}</span><span className="rounded-full bg-surface-container px-3 py-1">{carrosDisponiveisNoDia} carros disponíveis no dia · {planejamento.carrosDisponiveis} selecionados</span><span className="rounded-full bg-surface-container px-3 py-1">{dados.planos.filter((plano) => plano.situacao === "aprovado").length} planos aprovados</span></div>
      {eventoFoco && dados.veiculos.length === 0 && <p role="status" className="rounded-xl bg-surface-container p-3 text-sm text-on-surface-variant">Não há veículos cadastrados para sugerir neste evento. <a href="/admin/frota" className="font-semibold text-primary underline">Cadastrar carro na frota</a></p>}
      {compacto && !eventoFoco && <div className="flex flex-wrap gap-1.5 text-xs"><button type="button" aria-expanded={mostrarViagem} onClick={() => setMostrarViagem((atual) => !atual)} className="rounded-lg bg-surface-container px-2.5 py-1.5">Rota com paradas</button><button type="button" aria-expanded={mostrarCarros} onClick={() => setMostrarCarros((atual) => !atual)} className="rounded-lg bg-surface-container px-2.5 py-1.5">Filtrar carros</button><button type="button" aria-expanded={mostrarDupla} onClick={() => setMostrarDupla((atual) => !atual)} className="rounded-lg bg-surface-container px-2.5 py-1.5">Mesma equipe em 2 eventos</button></div>}
      {pendenciasExibidas.length > 0 && <div role="status" className="space-y-2 rounded-xl bg-primary/10 p-3 text-sm"><button type="button" aria-expanded={!compacto || mostrarPendencias} onClick={() => setMostrarPendencias((atual) => !atual)} className="text-left font-semibold">Ajuda necessária em {pendenciasExibidas.length} {pendenciasExibidas.length === 1 ? "evento" : "eventos"}</button>{(!compacto || mostrarPendencias) && <ul className="space-y-1">{pendenciasExibidas.map((pendencia) => <li key={pendencia.eventoId}><span className="font-semibold">{dados.eventos.find((evento) => evento.id === pendencia.eventoId)?.cliente_nome ?? "Evento"}:</span> {pendencia.motivo}</li>)}</ul>}</div>}
      {(!compacto || mostrarCarros) && <div className="space-y-2 rounded-xl bg-surface-container p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><strong>Carros usados nas sugestões</strong><div className="flex gap-2"><button type="button" onClick={() => setSelecionados(null)} className="rounded-lg bg-surface-container-high px-2 py-1">Todos</button><button type="button" onClick={() => setSelecionados(new Set())} className="rounded-lg bg-surface-container-high px-2 py-1">Limpar</button></div></div><div className="flex flex-wrap gap-x-4 gap-y-2">{dados.veiculos.map((carro) => <label key={carro.id} className="flex items-center gap-1"><input type="checkbox" checked={selecionados === null || selecionados.has(carro.id)} onChange={(ev) => { const proximo = new Set(selecionados ?? dados.veiculos.map((item) => item.id)); if (ev.target.checked) proximo.add(carro.id); else proximo.delete(carro.id); setSelecionados(proximo); }} /><span>{carro.modelo} · {carro.placa} · {carro.proprietario_id ? "particular" : "empresa"}</span></label>)}</div></div>}
      {!eventoFoco && eventosParaViagem.length >= 2 && (!compacto || mostrarViagem) && <section className="space-y-3 rounded-2xl bg-surface-container p-4" aria-label="Viagem com várias paradas">
        ...
        {sugestoesViagem.length > 0 && <div className="grid gap-2"><strong className="text-xs">Carros sugeridos para rota conjunta</strong>{sugestoesViagem.map(({ carro, grupo }) => <button key={carro.id} type="button" onClick={() => { setCarroViagem(carro.id); setMotoristaViagem(carro.proprietarioId ?? ""); setEventosViagem(grupo); }} className="rounded-xl bg-surface-container-lowest p-3 text-left text-xs hover:bg-primary-container"><span className="block font-semibold">{carro.modelo} · {carro.placa} · {grupo.length} paradas</span><span className="mt-1 block text-on-surface-variant">{grupo.map((id) => dados.eventos.find((item) => item.id === id)?.cliente_nome ?? "Evento").join(" → ")}</span></button>)}</div>}
      ...
      {eventosExibidos.length === 0 ? <p className="rounded-xl bg-surface-container p-5 text-sm text-on-surface-variant">Nenhum evento confirmado para este dia.</p> : <div className={compacto ? "grid gap-3" : "grid gap-3 md:grid-cols-2 xl:grid-cols-3"}>{eventosExibidos.map((evento) => {
        const requisito = dados.requisitos.find((item) => item.evento_id === evento.id);
        const rota = rotaAtual(dados, evento.id);
        const dupla = dados.duplos.find((item) => item.primeiro_evento_id === evento.id || item.segundo_evento_id === evento.id);
        ...
        return <article key={evento.id} className={compacto ? "rounded-xl bg-surface-container-lowest p-2.5 ring-1 ring-outline-variant/20" : "space-y-3 rounded-2xl bg-surface-container-lowest p-4 ring-1 ring-outline-variant/30"}>
          {compacto ? <button type="button" aria-expanded={eventoExpandido === evento.id} onClick={() => { setEventoExpandido((atual) => atual === evento.id ? null : evento.id); if (!rota && dados.locais.some((local) => local.evento_id === evento.id)) void acao("rota", { evento: evento.id }); }} className="flex w-full items-center justify-between gap-2 text-left text-xs"><span className="min-w-0"><strong className="block truncate text-sm">{evento.horario?.slice(0, 5)} · {evento.cliente_nome || "Evento"}</strong><span className="block truncate text-on-surface-variant">{rota ? `${rota.distancia_km} km · ${rota.duracao_minutos} min` : "Rota a calcular"} · {proposta ? dados.veiculos.find((carro) => carro.id === proposta.veiculoId)?.modelo ?? "Carro sugerido" : "Sem sugestão"}</span></span><span className="shrink-0 rounded-lg bg-surface-container px-2 py-1">{eventoExpandido === evento.id ? "Fechar" : "Planejar"}</span></button> : <div><h3 className="font-semibold">{evento.horario?.slice(0, 5)} · {evento.cliente_nome || "Evento"}</h3><p className="text-xs text-on-surface-variant">{(evento.inteiros ?? 0) + (evento.meios ?? 0)} convidados{dupla ? ` · Dupla ${dupla.codigo} · ${dupla.primeiro_evento_id === evento.id ? "1º evento" : "2º evento"}` : ""}</p></div>}
          {(!compacto || eventoExpandido === evento.id) && <div className="space-y-3 pt-2">
