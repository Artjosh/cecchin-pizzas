...
export function CadastroVeiculos({ veiculos, pessoas }: { veiculos: Veiculo[]; pessoas: Pessoa[] }) {
  ...
  async function salvar(evento: FormEvent) {
    ...
    try {
      ...
      setAberto(false); setEditando(null); router.refresh();
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível salvar"); }
    finally { setOcupado(false); }
  }
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-headline-sm text-headline-sm">Veículos cadastrados</h2><p className="text-sm text-on-surface-variant">Capacidade física; disponibilidade e escala são decididas por data.</p></div><button type="button" onClick={novo} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-on-primary"><Plus size={17} />Cadastrar veículo</button></div>
    ...
    {veiculos.length === 0 ? <p className="rounded-2xl bg-surface-container-low p-8 text-center text-on-surface-variant">Nenhum veículo físico cadastrado.</p> : <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{veiculos.map(v => <article key={v.id} className="rounded-2xl bg-surface-container-lowest p-4 shadow-sm ring-1 ring-outline-variant/30"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="rounded-xl bg-primary/10 p-2 text-primary"><CarFront size={22} /></span><div><h3 className="font-semibold">{v.modelo}</h3><p className="text-xs text-on-surface-variant">{v.placa} · {v.proprietario_id ? pessoas.find(p => p.id === v.proprietario_id)?.nome ?? "Veículo particular" : "Frota da empresa"}</p></div></div><button type="button" onClick={() => editar(v)} aria-label={`Editar ${v.modelo}`} className="rounded-lg bg-surface-container p-2 text-on-surface-variant hover:text-primary"><Pencil size={16} /></button></div><div className="mt-4 grid grid-cols-2 gap-2 text-sm"><span>Forno: {fornos[v.forno_maximo]}</span><span>Bebida: {bebidas[v.bebida_maxima]}</span><span>{v.lugares} lugares</span><span>{v.limite_eventos_levar} evento(s)/saída</span><span className={v.ativo ? "text-tertiary" : "text-on-surface-variant"}>{v.ativo ? "Ativo" : "Inativo"}</span></div></article>)}</div>}
  </div>;
}
