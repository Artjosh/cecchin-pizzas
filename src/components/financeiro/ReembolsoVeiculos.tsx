...
export type RegraVeiculoParticular = { lavagem_cada: number; lavagem_valor_centavos: number; bonus_cada: number; bonus_valor_centavos: number };
...
export function ReembolsoVeiculos({ planos, veiculos, usos, lavagens, resumos, pessoas, regra }: { planos: PlanoVeiculo[]; veiculos: VeiculoFinanceiro[]; usos: UsoVeiculo[]; lavagens: UsoVeiculo[]; resumos: ResumoUsoVeiculo[]; pessoas: { id: string; nome: string }[]; regra: RegraVeiculoParticular }) {
  ...
  async function carregarPagina(tipo: "viagens" | "reembolsos" | "lavagens", pagina: number) {
    ...
    try {
      const resposta = await fetch(`/api/operacao/financeiro/veiculos?tipo=${tipo}&pagina=${pagina}`, { cache: "no-store" });
  ...
  return <section className="space-y-3">
    <h2 className="flex items-center gap-2 font-headline-sm text-headline-sm"><CarFront size={20} />Veículos particulares <span className="rounded-full bg-primary px-2 text-sm text-on-primary" aria-label={`${pendentes.length}${usosAtuais.length > 25 ? " ou mais" : ""} reembolsos nesta página`}>{pendentes.length}{usosAtuais.length > 25 ? "+" : ""}</span></h2>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <form onSubmit={e => { e.preventDefault(); if (escolhido) void enviar({ acao: "registrar_uso_veiculo", plano: escolhido.id, km: Number(km), material, lavagem: lavagem || null }).then(ok => { if (ok) { setPlano(""); setKm(""); setLavagem(""); } }); }} className="grid content-start gap-3 rounded-2xl bg-surface-container-low p-4">
        ...
        {escolhido && <><p className="text-sm text-on-surface-variant">{pessoas.find(p => p.id === escolhido.motorista_id)?.nome ?? "Motorista"} · uso nº {numeroUso}</p><label className="text-sm">Quilômetros rodados (ida e volta)<input required className={`mt-1 w-full ${campo}`} type="number" min="0" max="3000" step="0.01" value={km} onChange={e => setKm(e.target.value)} /></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={material} onChange={e => setMaterial(e.target.checked)} />Transportou material</label>{numeroUso % regra.lavagem_cada === 0 && <label className="text-sm">Benefício a cada {regra.lavagem_cada} usos<select required className={`mt-1 w-full ${campo}`} value={lavagem} onChange={e => setLavagem(e.target.value)}><option value="">Escolha</option><option value="lavagem">Lavagem do carro</option><option value="dinheiro">{formatBRL(regra.lavagem_valor_centavos / 100)} em dinheiro</option></select></label>}{numeroUso % regra.bonus_cada === 0 && <p className="text-sm text-tertiary">Este uso também recebe bônus de {formatBRL(regra.bonus_valor_centavos / 100)}.</p>}</>}
        <button disabled={ocupado || !escolhido || km === "" || (numeroUso % regra.lavagem_cada === 0 && !lavagem)} className="rounded-xl bg-primary px-4 py-2 text-on-primary disabled:opacity-40">Registrar reembolso</button>
