...
| 14 | Instagram 🟡 | A central inclui ponte Chromium/Brave de leitura para o feed e Stories dos perfis seguidos na sessão oficial do navegador, mais OAuth e fila de publicação de uma imagem via API Meta. O Instagram já estava autenticado no perfil Brave. | A extensão `browser-extension/instagram-bridge` ainda precisa ser carregada e autorizada no navegador antes de trazer conteúdo à página. Publicação via API ainda exige App ID/Secret, callback HTTPS, chave AES, versão Graph e segredo Storage; worker Docker continua em imagem antiga. Não há prova de postagem. DMs e ações sociais não estão na ponte. |
